...
export function DemoPack() {
  ...
  return <main className={`demo-page demo-${subject}`}>
    ...
    {!session ? <div className="card demo-loading">{error || "Preparing the activities…"}</div> : question && <LearnerChrome demo><main className="learner-column demo-learner-column"><Achievement answered={Object.keys(results).length} correct={Object.values(results).filter(item => item.correct).length} total={questions.length} /><RaceTrack answered={Object.keys(results).length} total={questions.length} correct={Object.values(results).filter(item => item.correct).length} wrong={Object.values(results).filter(item => !item.correct).length} target={70} sleeping={Boolean(result && !result.correct)} /><div className="lesson-progress"><div><span>Today&apos;s trail</span><strong>{questions.indexOf(question) + 1} / {questions.length}</strong></div><i><b style={{ width: `${(questions.indexOf(question) / questions.length) * 100}%` }} /></i></div><div className="demo-layout">
