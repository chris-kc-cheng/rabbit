...
import type { FamilyLearner, Progress, Reward, Subject, User, WorksheetTopic } from "./types";
...
export function ParentView({ refreshKey, onPractice }: { refreshKey: number; onPractice: (learner: User) => Promise<void> }) {
  ...
  useEffect(() => { void refresh(); void api.getSubjects().then(setSubjects); void api.getWorksheetTopics().then(data => { setTopics(data); setTopicKey(current => data.some(topic => `${topic.subject}:${topic.id}` === current) ? current : (data[0] ? `${data[0].subject}:${data[0].id}` : "")); }).catch(error => setNotice(error instanceof Error ? error.message : "Could not load worksheet topics")); }, [refreshKey]);
  ...
  const saveDefaultSubject = async (learner: FamilyLearner, subject: string) => { try { await api.saveDefaultSubject(learner.id, subject); setNotice(`${learner.display_name}'s next question bank is set.`); await refresh(); } catch (error) { setNotice(error instanceof Error ? error.message : "Could not save the question bank"); } };
  const practice = async (learner: FamilyLearner) => { try { await onPractice(learner); } catch (error) { setNotice(error instanceof Error ? error.message : "Could not open learner practice"); } };
  ...
  const childCount = learners.filter(learner => !learner.is_self).length;
  return <main className="portal-page parent-dashboard"><header className="portal-hero"><div><p className="eyebrow">Family dashboard</p><h1>Learning at a glance</h1><p>See everyone together, choose each person&apos;s next question bank, or enter their learner view.</p></div><div className="family-count"><span>{childCount}</span><small>{childCount === 1 ? "child" : "children"}</small><button className="add-child" onClick={() => setShowAdd(true)} aria-label="Add a learner" title="Add a learner">＋</button></div></header>{notice && <p className="notice" role="status">{notice}</p>}
    <section className="children-accordion" aria-label="Learner progress">
      ...
      {learners.map(learner => { const progress = learner.progress; const isOpen = learner.id === openId; const totalTime = progress.attempt_history.reduce((sum, item) => sum + item.time_spent_ms, 0); const skillEvidence = Object.entries(progress.attempt_history.reduce<Record<string, { correct: number; total: number }>>((items, attempt) => { const item = items[attempt.skill] ?? { correct: 0, total: 0 }; item.total += 1; if (attempt.correct) item.correct += 1; items[attempt.skill] = item; return items; }, {})).map(([skill, evidence]) => ({ skill, ...evidence, accuracy: evidence.correct / evidence.total })).sort((a, b) => b.accuracy - a.accuracy); const strength = skillEvidence.find(item => item.total >= 2 && item.accuracy >= .8); const improvement = [...skillEvidence].reverse().find(item => item.total >= 2 && item.accuracy < .8); const skillName = (value: string) => value.split(".").slice(-2).join(" "); return <article className={`child-summary ${isOpen ? "open" : ""}`} key={learner.id}>
        <button className="child-summary-toggle" aria-expanded={isOpen} aria-controls={`child-${learner.id}`} onClick={() => setOpenId(isOpen ? "" : learner.id)}><span className="child-avatar" aria-hidden="true">{learner.display_name.slice(0, 1).toUpperCase()}</span><span className="child-name">{learner.display_name}<small>{learner.is_self ? "You · parent learner" : `@${learner.username}`}</small></span><span className="summary-stat"><i>{progress.attempts}</i><small>questions</small></span><span className="summary-stat"><i>{Math.round(progress.accuracy * 100)}%</i><small>accuracy</small></span><span className="summary-stat"><i>{streak(progress)}</i><small>day streak</small></span><span className="summary-stat"><i>{duration(totalTime)}</i><small>time spent</small></span><span className="accordion-chevron" aria-hidden="true">▼</span></button>
        {isOpen && <div className="child-detail" id={`child-${learner.id}`}><div className="child-overview"><ActivityChart progress={progress} /><section className="progress-ring" style={{ "--progress": `${Math.round(progress.accuracy * 100) * 3.6}deg` } as React.CSSProperties}><div><span>{Math.round(progress.accuracy * 100)}%</span><small>accuracy</small></div><p>{progress.correct} of {progress.attempts} answers correct</p></section><section className="quick-evidence"><h3>Practice signals</h3>{Object.keys(progress.misconceptions).length ? Object.entries(progress.misconceptions).slice(0, 3).map(([name, count]) => <p key={name}><span>{count}×</span>{name.split(".").slice(-2).join(" ")}</p>) : <p className="empty">No recurring signals yet.</p>}</section></div>
          ...
          <section className="next-bank"><label>Next question bank<select aria-label={`Default next question bank for ${learner.display_name}`} value={learner.default_subject ?? "math.elementary"} onChange={event => void saveDefaultSubject(learner, event.target.value)}>{subjects.map(subject => <option key={subject.id} value={subject.id}>{subject.title}</option>)}</select></label><button className="primary" onClick={() => void practice(learner)}>{learner.is_self ? "Start my practice" : `View as ${learner.display_name}`}</button></section>
          <nav className="dashboard-tools" aria-label={`Tools for ${learner.display_name}`}><button aria-pressed={openTool === "history"} onClick={() => setOpenTool(openTool === "history" ? null : "history")}><span aria-hidden="true">▤</span>Answers</button>{!learner.is_self && <button aria-pressed={openTool === "reward"} onClick={() => setOpenTool(openTool === "reward" ? null : "reward")}><span aria-hidden="true">★</span>Reward</button>}<button aria-pressed={openTool === "worksheet"} onClick={() => setOpenTool(openTool === "worksheet" ? null : "worksheet")}><span aria-hidden="true">▣</span>Worksheet</button>{!learner.is_self && <button onClick={reset}><span aria-hidden="true">●</span>Access</button>}</nav>
