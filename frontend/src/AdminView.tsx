...
export function AdminView({ onImpersonate }: { onImpersonate: (user: User) => Promise<void> }) {
  ...
  return <main className="admin-workspace">
    ...
    <div className="admin-main">
      ...
      {section === "users" && <div className="people-layout"><section className="admin-card"><div className="list-title"><h2>Accounts</h2><span>{accounts.length}</span></div><div className="people-table"><div className="table-head"><span>Person</span><span>Role</span><span>Status</span><span>Actions</span></div>{accounts.map(user => <div key={user.id}><span><strong>{user.display_name}</strong><small>@{user.username}</small></span><span className="role-label">{user.role}</span><span className={`user-status ${user.disabled ? "disabled" : "active"}`}>{user.disabled ? "Paused" : "Active"}</span><span><button className="quiet view-as" disabled={user.disabled} title={user.disabled ? "Activate this account before viewing as this user" : undefined} onClick={() => void impersonate(user)}>View as</button><button className="quiet" onClick={() => setEditingUser({...user})}>Edit</button><button className="quiet" onClick={() => void reset(user)}>Reset password</button></span></div>)}</div></section><section className="admin-card create-account"><p className="eyebrow">New account</p><h2>Create a parent</h2><form onSubmit={create} className="stack-form"><label>Display name<input name="name" autoComplete="name" required /></label><label>Username<input name="username" autoComplete="username" minLength={3} required /></label><label>Temporary password<input name="password" type="password" autoComplete="new-password" minLength={8} required /></label><button className="primary">Create parent</button></form></section></div>}
