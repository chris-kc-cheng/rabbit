...
export default function App() {
  ...
  return <div className="app-shell">
    <header className="topbar signed-in"><div className="brand"><img className="brand-logo" src="/rabbit-reading-logo.png" alt="" />rabbit</div><p className="role-chip">{user.role} space</p><div className="header-tools">{themeButton}<div className="account"><span>Hi, <strong>{user.display_name}</strong></span><button className="quiet" onClick={logout}>Log out</button></div></div></header>
    {user.role === "admin" ? <AdminView /> : user.role === "parent" ? <ParentView refreshKey={0} /> : <div className="learner-layout"><aside className="trail"><p className="eyebrow">Your path</p><h2>Math Explorer</h2><ol><li className="done">✓ <span>Ready<small>Signed in</small></span></li><li className="active">✦ <span>Mixed practice<small>In progress</small></span></li></ol></aside><LearnerView learnerId={user.id} onAttemptsChanged={() => {}} /><aside className="coach"><img className="mascot" src="/rabbit-reading-logo.png" alt="Rabbit reading" /><div><strong>You&apos;ve got this!</strong><p>Every thoughtful try makes your learning stronger.</p></div></aside></div>}
  </div>;
}
