...
export default function App() {
  ...
  return <div className="app-shell">
    <header className="topbar">
      ...
      <nav aria-label="Choose experience"><button className={view === "learner" ? "active" : ""} onClick={() => setView("learner")}>Learner</button><button className={view === "demo" ? "active" : ""} onClick={() => setView("demo")}>Explore packs</button><button className={view === "parent" ? "active" : ""} onClick={() => setView("parent")}>Parent preview</button><button className={view === "admin" ? "active" : ""} onClick={() => setView("admin")}>Admin</button></nav>
    ...
    </div> : view === "demo" ? <DemoPack /> : view === "parent" ? <ParentView refreshKey={refreshKey} /> : <AdminView onChanged={() => setRefreshKey(key => key + 1)} />}
