...
- [~] **Rewards:** parents can enable a target and name a present/experience, but
  data is kept only in backend memory, resets on restart, and does not yet support
  the confirmed per-child lifetime balance or enabled-by-default deductions.
